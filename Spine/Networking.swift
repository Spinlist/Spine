...
open class HTTPClient: NetworkClient {
	...
	*/
    ...
    //open func request(_ method: String, URL: URL, callback: (Int?, Data?, NSError?) -> Void) {
    //    <#code#>
    //}
    public func request(_ method: String, URL: URL, payload: Data?, callback: @escaping (Int?, Data?, NSError?) -> Void) {
        ...
        let task = urlSession.dataTask(with: request as URLRequest) { (data, response, networkError) in
